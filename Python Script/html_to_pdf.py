...
for name in os.listdir(args.inputDir):
    if ".html" in name:
        ...
        try:
            if args.css is None:
                pdfkit.from_file(f"{_input}", f"{_output}.pdf", configuration=config)
            else:
                pdfkit.from_file(f"{_input}", f"{_output}.pdf", configuration=config, css=css)
            print(Fore.GREEN + "Succes!")
        except Exception as e:
            print(Fore.YELLOW + f"{e.args[-1].split()[-1]} while converting.")
